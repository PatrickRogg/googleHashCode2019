...
public class Generator {
	...
	private static final int MAX_VERTICAL_LOOPS = 2;

    private Scorer scorer = new Scorer();

	public List<Slide> createSlideshow(List<Foto> fotos) {
		List<Foto> verticalFotos = new ArrayList<>();
		List<Foto> horizontalFotos = new ArrayList<>();
	...
	private List<Slide> getOptimalSlides(List<Slide> slides) {
		...
		for(int i = 0; i < slidesSize; i++) {
			...
			Set<Slide> allSlidesThatContainTagOfSlide = new HashSet<>();
			...
			List<Slide> innerLoopSlides = new ArrayList<>(allSlidesThatContainTagOfSlide);
	...
	private Map<String, List<Slide>> getSlidesOfTag(Collection<Slide> slides) {
		Map<String, List<Slide>> slidesOfTag = new HashMap<>();
	...
	private List<Slide> getVerticalFotoSlides(List<Foto> verticalFotos) {
		...
		while (verticalFotos.size() > 0) {
			...
			while (innerLoopFotos.size() > 0) {
				counter++;
				if (optimalFotoDuplications == 0 || counter > MAX_VERTICAL_LOOPS) {
					break;
				}
